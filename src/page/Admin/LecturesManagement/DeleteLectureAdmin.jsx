...
const DeleteLectureAdmin = ({ show, handleClose, lectureData, fetchAllLectures }) => {
  const { toast } = useToast();
  console.log("user deleted", lectureData);
  const confirmDelete = async () => {
    try {
      const res = await deleteLecture(lectureData?.id);
      console.log("res", res);
      ...
      <DialogContent className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg mx-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2 text-xl font-semibold text-gray-900">
            ...
            <span>Xác nhận xóa bài giảng!</span>
          </DialogTitle>
...
export default DeleteLectureAdmin;
