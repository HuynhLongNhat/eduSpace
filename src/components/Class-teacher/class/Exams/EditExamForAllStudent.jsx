...
import { editExamByExamId, getExamDetail } from "@/api/ExamApi";
...
const EditExamForAllStudent = () => {
    ...
    const fetchExamData = async () => {
      try {
        ...
        form.reset({
          ...
          type: examData.type,
          due_date: new Date(examData.due_date),
  ...
  const onSubmit = async (values) => {
    try {
      ...
      const response = await editExamByExamId(examId, examData);

      if (response.success) {
        toast.success(response.message || "Chỉnh sửa bài kiểm tra thành công");
        navigate(-1);
      }
    } catch (error) {
      console.error(error);
              ...
              <div className="flex items-center space-x-2">
                <div className="flex flex-col">
                  ...
                  <p className="text-gray-600 mt-2 dark:text-white">
                    Điền thông tin để chỉnh sửa thông tin bài tập
                  </p>
                </div>
              </div>
              <div className="flex dark:text-white items-center space-x-2 text-sm text-gray-500">
...
export default EditExamForAllStudent;
