...
import { addFileToLectureByIdAndClassId } from "@/api/LectureApi";
...
const AddFileOfLecture = ({
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      // Gọi API cho từng file (cấu trúc API có thể điều chỉnh nếu cần)
      for (let uploadResult of uploadResults) {
        const content = uploadResult.url;
        const response = await addFileToLectureByIdAndClassId(lectureId , classId, {
          content,
        });
        if (!response.success) {
...
export default AddFileOfLecture;
