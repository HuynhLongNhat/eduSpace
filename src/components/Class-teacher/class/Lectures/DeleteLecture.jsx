...
import { deleteLectureByIdAndClassId } from "@/api/LectureApi";
...
const DeleteLecture = ({
  ...
  handleClose,
  lectureToDelete,
  fetchAllLecturesByClassId,
}) => {
  const [isLoading, setIsLoading] = useState(false);
  const { classId } = useParams();
  const confirmDelete = async () => {
    setIsLoading(true);
    console.log("res", lectureToDelete?.lecture_id, classId);

    try {
      const res = await deleteLectureByIdAndClassId(
        lectureToDelete?.lecture_id,
        classId
      );
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2 text-xl font-semibold text-gray-900">
            ...
            <span className="dark:text-white">
              Xác nhận xóa {lectureToDelete.title}!
            </span>
          </DialogTitle>
